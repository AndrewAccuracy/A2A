...
# 三个领域
domain_list = ["data/question/art.txt"]
# 三个问题
question_index_list = [1]
# 两个隐写算法
stego_algorithm_list = ["binary_based"]#
# 两个模型
stego_model_list = ["/root/autodl-tmp/Llama-3.2-3B-Instruct"]#,"/root/autodl-tmp/Qwen2-7B-Instruct"]
# 两个密文，512和1024bits
secret_message_list = ["data/stego/secret_bits_1024.txt"]
...
def run_evaluation(session_id):
    ...
    try:
        # 构造对话文件路径
        conversation_file = f"data/conversation/conversation_{session_id}.json"
        ...
        evaluation_cmd = [
            ...
            "--evaluation_conversation", conversation_file
        ]
        
        print("启动评估模块...")
        evaluation_result = subprocess.run(evaluation_cmd, capture_output=True, text=True, timeout=300)
        
        print("评估输出:")
        print(evaluation_result.stdout)
        if evaluation_result.stderr:
            print("评估错误:")
            print(evaluation_result.stderr)
        
        return evaluation_result.returncode == 0
    except Exception as e:
        print(f"评估模块执行出错: {e}")
        return False
