...
from .uni_stego import uni_binary_dec, uni_cyclic_shift_dec

### time-step decoding

def differential_based_decoder(prob, indices, prev, PRG, precision):
    ...
    idx = (bin == prev).nonzero().item()
    bits = uni_cyclic_shift_dec(idx = idx, n = len(bin), PRG = PRG, precision=precision)

    return bits

def binary_based_decoder(prob, indices, prev, PRG, precision):
    if (prob[0] == 1):
        return ''
...
def stability_based_decoder(prob, indices, prev, PRG, precision):
...
def decoder(alg, prob, indices, prev,PRG, precision):
    if alg.lower() == "differential_based":
        return differential_based_decoder(prob, indices, prev,PRG, precision)
    if alg.lower() == "binary_based":
        return binary_based_decoder(prob, indices, prev,PRG, precision)
    if alg.lower() == "stability_based":
        return stability_based_decoder(prob, indices, prev,PRG, precision)
